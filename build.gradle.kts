...
tasks.register("dockerImageNativeNoTest") {group = "build"; dependsOn("bootJar", "vaadinBuildFrontend")
	...
	doFirst {
		exec { commandLine(
			...
			native-image -J-Xmx5700m -march=compatibility -H:Name=application --initialize-at-build-time=org.apache.commons.logging.LogFactory $([[ -f META-INF/native-image/argfile ]] && echo @META-INF/native-image/argfile) -cp .:BOOT-INF/classes:$(ls -d -1 "/build/native/nativeCompile/BOOT-INF/lib/"*.* | tr "\n" ":") && /build/native/nativeCompile/application -check-integrity """
		)}
