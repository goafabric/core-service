...
import org.goafabric.core.ui.adapter.vo.Patient;

import java.util.ArrayList;

public class MRCView extends VerticalLayout {

    private final SearchLogic<Patient> patientLogic;
    private final SearchLogic<Diagnosis> diagnosisLogic;
    private final SearchLogic<ChargeItem> chargeItemLogic;

    public MRCView(SearchLogic<Patient> patientLogic, SearchLogic<Diagnosis> diagnosisLogic, SearchLogic<ChargeItem> chargeItemLogic) {
    ...
    private void addMasterFilter() {
        ...
        masterFilter.setItems((CallbackDataProvider.FetchCallback<String, String>) query -> {
            ...
            var lastNames = filter.equals("")
                    ...
                    : patientLogic.search(filter).stream().map(p -> p.familyName()).limit(query.getLimit());
